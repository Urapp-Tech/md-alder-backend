...
const list = async (req, params) => {
  ...
  const query = knex
    .from(MODULE.ADMIN.OPERATION_CATEGORY)
    .where({
...
const create = async (req, params) => {
  ...
  const existingOperationCat = await transaction(
    MODULE.ADMIN.OPERATION_CATEGORY
  )
    .where((builder) => {
      builder
  ...
  const existingOperationCat = await transaction(
    ...
    .where((builder) => {
      builder
        ...
        .andWhere({ branch: params.branch })
        .andWhere({ isDeleted: false })
        .andWhereNot('id', params.operationCategoryId);
    })
    .first();
